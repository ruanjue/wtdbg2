...
static inline u8i filter_reads_kbm(KBM *kbm, u8i retain_size, int strategy){
	...
	retain_size = kbm_cvt_length(retain_size);
	...
	if((kbm->flags & 0x2) == 0){
		...
		if(strategy == 0){ // longest
			...
			for(;e<kbm->reads->size;e++){
				kbm->reads->buffer[e].closed = 1;
			}
			...
			e = b * 2;
			b = m - b;
			while(b) kbm->reads->buffer[--b].closed = 1;
			while(e < kbm->reads->size) kbm->reads->buffer[e++].closed = 1;
		} else {
			len = kbm->rdseqs->size;
		}
		return len;
...
static inline void ready_kbm(KBM *kbm){
	...
	renew_kbmbinv(kbm->bins, (kbm->rdseqs->size >> KBM_BIN_BITS));
	kbm->avail_bases = 0;
	kbm->avail_reads = 0;
	for(i=0;i<kbm->reads->size;i++){
		...
		if(rd->closed == 0){
			kbm->avail_reads ++;
			kbm->avail_bases += rd->bincnt * KBM_BSIZE;
		}
	}
	clear_bitvec(kbm->binmarks);
